...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, query_mask=None):
        # Get attention score
        attn = t.bmm(query, key.transpose(1, 2))   # t.bmm：batch matrix multiply（）
        attn = attn / math.sqrt(self.num_hidden_k)


        # Masking to ignore padding (key side)
        if mask is not None:
            attn = attn.masked_fill(mask, -2 ** 32 + 1)   #是将mask中为1的元素所在的索引，在a中相同的的索引处替换为value
...
class Attention(nn.Module):
    ...
    def forward(self, memory, decoder_input, mask=None, query_mask=None):
        #memory:x  decoder_input:x
        batch_size = memory.size(0)
        seq_k = memory.size(1)
        #print('klkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk',seq_k)
        ...
        #如果我们在 transpose、permute 操作后执行 view，Pytorch 会抛出错误
        # contiguous转成连续数组
