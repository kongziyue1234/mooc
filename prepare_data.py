...
if __name__ == '__main__':
    # dataset = PrepareDataset(os.path.join('E:\data\\news','label_Pinyin.csv'),os.path.join('E:\data\\news','wavs'))
    ...
    dataset = PrepareDataset(os.path.join('D:\陈曦\dataset','result2_pinyin.csv'),os.path.join('D:\陈曦\dataset\muke'))
    #dataset = PrepareDataset(os.path.join(hp.data_path,'metadata.csv'), os.path.join(hp.data_path,'wavs'))
    dataloader = DataLoader(dataset, batch_size=1, drop_last=False, num_workers=8)
    from tqdm import tqdm
    pbar = tqdm(dataloader)
