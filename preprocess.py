...
class LJDatasets(Dataset):
    ...
    def __init__(self, csv_file, root_dir):
        ...
        """
        self.landmarks_frame = pd.read_csv(csv_file, sep=':', header=None, dtype=str)
    ...
    def __getitem__(self, idx):
        ...
        #print('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA======',len(text))
        text = np.asarray(text_to_sequence(text, [hp.cleaners]), dtype=np.int32)
        #print(text)
        #print('BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB========',len(text))
        mel = np.load(os.path.join(self.root_dir, self.landmarks_frame.ix[idx, 0]) + '.pt.npy')
        #print(mel.shape)
        mel_input = np.concatenate([np.zeros([1,hp.num_mels], np.float32), mel[:-1,:]], axis=0)
        text_length = len(text)
        # print('text_lengthtext_length',text_length)
        pos_text = np.arange(1, text_length + 1)
        #print('pos_textpos_textpos_text',pos_text)
        pos_mel = np.arange(1, mel.shape[0] + 1)
        # print('mel.shape[0]mel.shape[0]mel.shape[0]',mel.shape[0])
...
class PostDatasets(Dataset):
    ...
    def __getitem__(self, idx):
        #wav_name = os.path.join(self.root_dir, self.landmarks_frame.ix[idx, 0]) + '.wav'
        mel = np.load(os.path.join(self.root_dir, self.landmarks_frame.ix[idx, 0])+'.pt.npy')
        mag = np.load(os.path.join(self.root_dir, self.landmarks_frame.ix[idx, 0])+'.mag.npy')
...
def get_dataset():
    return LJDatasets(os.path.join('D:\陈曦\dataset','result2_pinyin.csv'), os.path.join('D:\陈曦\dataset\muke'))

def get_post_dataset():
    return PostDatasets(os.path.join('D:\陈曦\dataset','result2_pinyin.csv'),os.path.join('D:\陈曦\dataset\muke'))
